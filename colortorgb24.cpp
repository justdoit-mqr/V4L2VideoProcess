...
//静态成员数组声明(存放在静态存储区，未初始化存放在bss段，不占实际的磁盘空间)
uchar ColorToRgb24::r_yv_table[256][256];
uchar ColorToRgb24::g_yuv_table[256][256][256];
uchar ColorToRgb24::b_yu_table[256][256];
#endif
...
#ifdef USE_RGB_YUV_TABLE
/*
 *@brief:  根据整形移位转换公式，使用yuv的不同组合初始化rgb表
 *@date:   2024.03.22
 */
void ColorToRgb24::initRgbTableFromYuv()
{
    //qDebug()<<"initRgbTableFromYuv-start:"<<QTime::currentTime().toString("hh:mm:ss:zzz");
    int r,g,b;
    for(int y=0;y<256;y++)
    {
        for(int u=0;u<256;u++)
        {
            b = y+(u-128)+((197*(u-128))>>8);
            b_yu_table[y][u] = (b > 255)?255:(b < 0)?0:b;
            for(int v=0;v<256;v++)
            {
                g = y-((88*(u-128))>>8)-((183*(v-128))>>8);
                g_yuv_table[y][u][v] = (g > 255)?255:(g < 0)?0:g;
            }
        }
        for(int v=0;v<256;v++)
        {
            r = y+(v-128)+((103*(v-128))>>8);
            r_yv_table[y][v] = (r > 255)?255:(r < 0)?0:r;
        }
    }
    //qDebug()<<"initRgbTableFromYuv-end:"<<QTime::currentTime().toString("hh:mm:ss:zzz");
}
/*
 *@brief:   将yuyv帧格式数据转换成rgb24格式数据，这里采用的是查表法
 ...
 */
void ColorToRgb24::yuyv_to_rgb24_table(uchar *yuyv, uchar *rgb24, const uint &width, const uint &height)
{
    //qDebug()<<"yuyv_to_rgb24_table-start:"<<QTime::currentTime().toString("hh:mm:ss:zzz");
    ...
    for(int i = 0;i<yvyuLen;i += 4)
    {
        ...
        y1 = yuyv[i+2];
        v  = yuyv[i+3];
        rgb24[rgbIndex++] = r_yv_table[y0][v];
        rgb24[rgbIndex++] = g_yuv_table[y0][u][v];
        rgb24[rgbIndex++] = b_yu_table[y0][u];
        rgb24[rgbIndex++] = r_yv_table[y1][v];
        rgb24[rgbIndex++] = g_yuv_table[y1][u][v];
        rgb24[rgbIndex++] = b_yu_table[y1][u];
    }
    //qDebug()<<"yuyv_to_rgb24_table-end:"<<QTime::currentTime().toString("hh:mm:ss:zzz");
}
/*
 *@brief:   将NV12或者NV21帧格式数据转换成rgb24格式数据，这里采用的是查表法
 ...
 *@param:   width:宽度  height:高度
 */
void ColorToRgb24::nv12_21_to_rgb24_table(bool is_nv12, uchar *nv12_21, uchar *rgb24,
                                          const uint &width, const uint &height)
{
    //qDebug()<<"nv12_21_to_rgb24_table-start:"<<QTime::currentTime().toString("hh:mm:ss:zzz");
    ...
    for(uint i=0;i<height;i+=2)//一次处理两行
    {
        ...
        for(uint j=0;j<width;j+=2)//一次处理两列
        {
            ...
            if(is_nv12)
            {
                u = nv12_21[y_len-(cur_row_pixel_len>>1)+cur_pixel_pos];
                v = nv12_21[y_len-(cur_row_pixel_len>>1)+cur_pixel_pos+1];
            }
            else
            {
                v = nv12_21[y_len-(cur_row_pixel_len>>1)+cur_pixel_pos];
                u = nv12_21[y_len-(cur_row_pixel_len>>1)+cur_pixel_pos+1];
            }
            //四个Y分量，共用一组uv
            ...
            //计算奇数行rgb
            rgb24[cur_pixel_rgb_pos] = r_yv_table[y_odd1][v];
            rgb24[cur_pixel_rgb_pos+1] = g_yuv_table[y_odd1][u][v];
            rgb24[cur_pixel_rgb_pos+2] = b_yu_table[y_odd1][u];
            rgb24[cur_pixel_rgb_pos+3] = r_yv_table[y_odd2][v];
            rgb24[cur_pixel_rgb_pos+4] = g_yuv_table[y_odd2][u][v];
            rgb24[cur_pixel_rgb_pos+5] = b_yu_table[y_odd2][u];
            //计算偶数行rgb
            rgb24[next_pixel_rgb_pos] = r_yv_table[y_even1][v];
            rgb24[next_pixel_rgb_pos+1] = g_yuv_table[y_even1][u][v];
            rgb24[next_pixel_rgb_pos+2] = b_yu_table[y_even1][u];
            rgb24[next_pixel_rgb_pos+3] = r_yv_table[y_even2][v];
            rgb24[next_pixel_rgb_pos+4] = g_yuv_table[y_even2][u][v];
            rgb24[next_pixel_rgb_pos+5] = g_yuv_table[y_even2][u][v];
    ...
    //qDebug()<<"nv12_21_to_rgb24_table-end:"<<QTime::currentTime().toString("hh:mm:ss:zzz");
}
#endif
/*
