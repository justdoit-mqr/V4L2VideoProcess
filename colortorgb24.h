 ...
 *
 *注:初期尝试过使用完全查表法(提前基于转换公式将r、g、b的所有可能性计算出来存到表里，通过yuv值索引获取)实现yuv到rgb的转换，但该方式会
 *涉及多维数据（r_yv_table[256][256]、g_yuv_table[256][256][256]、b_yu_table[256][256])访问,初始化运算量较大(进行一千六百多
 *万次的计算，需要考虑cpu主频性能，否则初始化时间可能难以接受)，占用内存较多(约16M)，更严重的是完全查表法有一个无法解决的性能瓶颈，便是
 *多维数组数据量太大造成cpu缓存命中率降低，导致频繁读内存查表效率反而更低。为了避开该瓶颈，采用部分查表法(提前基于整形移位将uv分量跟rgb
 *分量的关系计算存到表里)省却了一部分cpu计算，但后续还是要关联Y分量，进行rgb阈值判断，相较与对每个像素都进行整形移位计算的方式，部分查表
 *法在性能上并没有提高多少。
 */
